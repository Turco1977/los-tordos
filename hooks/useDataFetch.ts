    ...
    setTimeout(async () => {
      try {
        ...
        if (!batch3Loaded.current) {
          const [b3inv, b3sp, b3pb, b3im, b3id, b3sd, b3arch, b3tn, b3th, b3tc, b3fx, b3bec, b3asc, b3tar, b3sc, b3pipe, b3ct, b3prop, b3pv, b3pm, b3hi, b3mat, b3pag, b3tmsg, b3pmsg] = await Promise.all([
            ...
            supabase.from("project_messages").select("*").order("created_at"),
          ]);
