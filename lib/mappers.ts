...
export const profileToUser = (p: Profile) => ({ id: p.id, n: p.first_name, a: p.last_name, role: p.role, dId: p.dept_id, div: p.division, mail: p.email, tel: p.phone });
...
export const taskToDB = (p: any): Partial<Task> => ({ division: p.div || "", creator_id: p.cId, creator_name: p.cN, dept_id: p.dId, tipo: p.tipo, description: p.desc, due_date: p.fReq, urgency: p.urg, status: p.st, assigned_to: p.asTo, requires_expense: p.rG, expense_ok: p.eOk, resolution: p.resp, created_at: p.cAt, amount: p.monto });
