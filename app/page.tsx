...
function Login({users,deptos,onSel}:{users:any[];deptos:any[];onSel:any}){
  ...
  const logo=<><div style={{width:72,height:72,borderRadius:"50%",background:"rgba(255,255,255,.12)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",border:"2px solid rgba(255,255,255,.2)"}}><span style={{fontSize:36}}>🏉</span></div><h1 style={{color:"#fff",fontSize:26,margin:"0 0 2px",fontWeight:800}}>LOS TORDOS</h1><p style={{color:"rgba(255,255,255,.45)",fontSize:11,margin:"0 0 28px",letterSpacing:2,textTransform:"uppercase" as const}}>Rugby Club · Sistema de Gestión v13.3</p></>;
  ...
  return(<div style={{maxWidth:680}}><h2 style={{margin:"0 0 4px",fontSize:19,color:T.nv,fontWeight:800}}>Organigrama</h2><p style={{color:T.g4,fontSize:12,margin:"0 0 18px"}}>Estructura institucional Los Tordos Rugby Club</p>
    ...
    <div style={{marginLeft:24,borderLeft:"2px solid "+T.nv+"22",paddingLeft:14}}>
      ...
      <div style={{marginLeft:24,borderLeft:"2px solid "+T.rd+"22",paddingLeft:14}}>
        {areas.filter((ar:any)=>ar.id!==100&&ar.id!==101).map((ar:any)=>{const ds=deptos.filter((d:any)=>d.aId===ar.id);return(<OrgNode key={ar.id} icon={ar.icon} title={ar.name} sub={ds.length+" deptos"} color={ar.color} ex={!!ex["ar"+ar.id]} onTog={()=>tog("ar"+ar.id)} cnt={ds.length}>{ds.map((d:any)=>{const pp=users.filter((u:any)=>u.dId===d.id);return(<OrgNode key={d.id} icon="📂" title={d.name} sub={pp.length+" personas"} color={ar.color} ex={!!ex["d"+d.id]} onTog={()=>tog("d"+d.id)} cnt={pp.length}>{pp.length>0?pp.map((u:any)=>(<div key={u.id} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",background:"#fff",borderRadius:6,border:"1px solid "+T.g2,marginBottom:2}}><span style={{fontSize:11}}>{ROLES[u.role]?.i}</span><div><div style={{fontSize:11,fontWeight:600,color:T.nv}}>{fn(u)}</div><div style={{fontSize:9,color:T.g4}}>{ROLES[u.role]?.l}{u.div?" · "+u.div:""}</div></div></div>)):<div style={{fontSize:10,color:T.g4,fontStyle:"italic",padding:4}}>Sin integrantes</div>}</OrgNode>);})}</OrgNode>);})}
      </div>
    </div>
  </div>);
}
