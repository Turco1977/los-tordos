  ...
  const renderLoteDet=()=>{
    ...
    return(<div>
      ...
      {distForm&&<Card style={{marginBottom:12,background:isDark?"rgba(139,92,246,.08)":"#F5F3FF",border:"1px solid "+colors.pr+"33",padding:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr",gap:8,marginBottom:8}}>
          <div><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>División *</label><select value={distForm.division} onChange={e=>{const div=e.target.value;const eu=enlaceUsers(div);const auto=eu.length===1?eu[0]:null;sDistForm((f:any)=>({...f,division:div,enlace_id:auto?auto.id:"",enlace_name:auto?fn(auto):""}));}} style={{...iS,marginTop:2}}><option value="">Seleccionar...</option>{DIV.map(d=><option key={d} value={d}>{d}</option>)}</select></div>
