...
export default function PresView({user,onAddPresu,onUpdPresu,onDelPresu,onAddProv,onSel,mob,UserPicker}:any){
  ...
  if(tab==="todos"){
    ...
    return(<div style={{maxWidth:800}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:4}}>
        <div><h2 style={{margin:"0 0 4px",fontSize:mob?16:19,color:colors.nv,fontWeight:800}}>💰 Presupuestos</h2><p style={{color:colors.g4,fontSize:12,margin:0}}>Repositorio de cotizaciones y comparación de proveedores</p></div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:12,marginTop:10}}>{[{k:"todos",l:"📋 Todos"},{k:"provs",l:"🏢 Proveedores"},{k:"kpis",l:"📊 KPIs"}].map(t=><button key={t.k} onClick={()=>sTab(t.k)} style={{padding:"6px 14px",borderRadius:8,border:"none",background:tab===t.k?colors.pr:cardBg,color:tab===t.k?"#fff":colors.g5,fontSize:12,fontWeight:600,cursor:"pointer"}}>{t.l}</button>)}</div>
      {/* Add presupuesto form */}
      {addMode&&canManage&&<Card style={{marginBottom:14,background:"#F5F3FF",border:"1px solid "+colors.pr+"33"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontSize:12,fontWeight:700,color:colors.pr}}>➕ Nuevo presupuesto</div><button onClick={resetNpf} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:colors.g4}}>✕</button></div>
        <div style={{marginBottom:8}}><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>Tarea vinculada *</label><select value={npf.task_id} onChange={e=>sNpf(p=>({...p,task_id:e.target.value}))} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+colors.g3,fontSize:12,marginTop:2}}><option value="">Seleccionar tarea...</option>{peds.filter((p:any)=>p.st!=="ok").map((p:any)=><option key={p.id} value={p.id}>#{p.id} – {p.tipo}: {p.desc.slice(0,50)}</option>)}</select></div>
        <div style={{marginBottom:8}}><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>Proveedor *</label>
          <input value={provSearch2} onChange={e=>{sProvSearch2(e.target.value);sNpf(p=>({...p,prov_nombre:e.target.value,prov_id:""}));}} placeholder="Buscar o escribir proveedor..." style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+colors.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:2}}/>
          {provSearch2&&(provs||[]).filter((pv:any)=>pv.nombre.toLowerCase().includes(provSearch2.toLowerCase())).length>0&&<div style={{border:"1px solid "+colors.g3,borderRadius:8,marginTop:2,maxHeight:100,overflowY:"auto" as const,background:cardBg}}>
            {(provs||[]).filter((pv:any)=>pv.nombre.toLowerCase().includes(provSearch2.toLowerCase())).map((pv:any)=><div key={pv.id} onClick={()=>{sNpf(p=>({...p,prov_id:String(pv.id),prov_nombre:pv.nombre,prov_contacto:pv.contacto||pv.telefono||pv.email}));sProvSearch2(pv.nombre);}} style={{padding:"6px 10px",fontSize:11,cursor:"pointer",borderBottom:"1px solid "+colors.g1}}>{pv.nombre} <span style={{color:colors.g4}}>({pv.rubro})</span></div>)}
          </div>}
        </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap" as const,alignItems:"center"}}>
        <input value={search} onChange={e=>sSr(e.target.value)} placeholder="🔍 Buscar..." style={{padding:"5px 10px",borderRadius:8,border:"1px solid "+colors.g3,fontSize:11,width:140}}/>
        <select value={fSt} onChange={e=>sFSt(e.target.value)} style={{padding:"5px 8px",borderRadius:8,border:"1px solid "+colors.g3,fontSize:11}}>
          <option value="all">Todos los estados</option>{Object.keys(PSC).map(k=><option key={k} value={k}>{PSC[k].i} {PSC[k].l}</option>)}
        </select>
        <select value={fArea} onChange={e=>sFArea(e.target.value)} style={{padding:"5px 8px",borderRadius:8,border:"1px solid "+colors.g3,fontSize:11}}>
          <option value="">Todas las áreas</option>{areas.map((a:any)=><option key={a.id} value={a.id}>{a.icon} {a.name}</option>)}
        </select>
        <input value={fProv} onChange={e=>sFProv(e.target.value)} placeholder="Proveedor..." style={{padding:"5px 10px",borderRadius:8,border:"1px solid "+colors.g3,fontSize:11,width:120}}/>
      </div>
      {/* Summary */}
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {Object.keys(PSC).map(k=>{const cnt=presu.filter((pr:any)=>pr.status===k).length;return <span key={k} onClick={()=>sFSt(fSt===k?"all":k)} style={{padding:"3px 10px",borderRadius:14,background:fSt===k?PSC[k].bg:cardBg,border:"1px solid "+(fSt===k?PSC[k].c:colors.g3),fontSize:10,fontWeight:600,color:PSC[k].c,cursor:"pointer"}}>{PSC[k].i} {cnt}</span>;})}
      </div>
      {/* List */}
  ...
  if(tab==="provs"){
    return(<div style={{maxWidth:800}}>
      <h2 style={{margin:"0 0 4px",fontSize:mob?16:19,color:colors.nv,fontWeight:800}}>💰 Presupuestos</h2>
      <p style={{color:colors.g4,fontSize:12,margin:"0 0 14px"}}>Repositorio de cotizaciones y comparación de proveedores</p>
      <div style={{display:"flex",gap:4,marginBottom:12}}>{[{k:"todos",l:"📋 Todos"},{k:"provs",l:"🏢 Proveedores"},{k:"kpis",l:"📊 KPIs"}].map(t=><button key={t.k} onClick={()=>sTab(t.k)} style={{padding:"6px 14px",borderRadius:8,border:"none",background:tab===t.k?colors.pr:cardBg,color:tab===t.k?"#fff":colors.g5,fontSize:12,fontWeight:600,cursor:"pointer"}}>{t.l}</button>)}</div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:700,color:colors.nv}}>🏢 Directorio de Proveedores ({provs.length})</div>
        {canManage&&<Btn v="pu" s="s" onClick={()=>sPvMode(pvMode==="add"?"list":"add")}>{pvMode==="add"?"✕ Cancelar":"+ Nuevo proveedor"}</Btn>}
      </div>
      {pvMode==="add"&&<Card style={{marginBottom:12,background:"#F5F3FF",border:"1px solid "+colors.pr+"33"}}>
        <div style={{fontSize:12,fontWeight:700,color:colors.pr,marginBottom:8}}>➕ Nuevo proveedor</div>
        <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:8,marginBottom:8}}>
          <div><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>Nombre *</label><input value={pvF.nombre} onChange={e=>sPvF(p=>({...p,nombre:e.target.value}))} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+colors.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:2}}/></div>
          <div><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>Contacto</label><input value={pvF.contacto} onChange={e=>sPvF(p=>({...p,contacto:e.target.value}))} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+colors.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:2}}/></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr",gap:8,marginBottom:8}}>
          <div><label style={{fontSize:10,fontWeight:600,color:colors.g5}}>Email</label><input value={pvF.email} onChange={e=>sPvF(p=>({...p,email:e.target.value}))} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+colors.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:2}}/></div>
      ...
      {provs.map((pv:any)=>{const pvPresu=presu.filter((pr:any)=>pr.proveedor_id===pv.id);const aprobados=pvPresu.filter((pr:any)=>pr.status===PST.APR);const totalApr=aprobados.reduce((s:number,pr:any)=>s+Number(pr.monto),0);
        return(<Card key={pv.id} style={{padding:"10px 14px",marginBottom:6,borderLeft:"3px solid "+colors.pr}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"start"}}>
            <div><div style={{fontSize:13,fontWeight:700,color:colors.nv}}>{pv.nombre}</div><div style={{fontSize:10,color:colors.g4}}>{pv.rubro}{pv.contacto?" · "+pv.contacto:""}</div></div>
            <div style={{textAlign:"right" as const}}><div style={{fontSize:10,color:colors.g5}}>Cotizaciones: {pvPresu.length}</div>{totalApr>0&&<div style={{fontSize:12,fontWeight:700,color:colors.gn}}>✅ ${totalApr.toLocaleString()}</div>}</div>
          </div>
          <div style={{display:"flex",gap:8,marginTop:4,fontSize:10,color:colors.g5}}>{pv.email&&<span>📧 {pv.email}</span>}{pv.telefono&&<span>📞 {pv.telefono}</span>}</div>
        </Card>);})}
    </div>);
  }
  ...
  if(tab==="kpis"){
    ...
    return(<div style={{maxWidth:800}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:mob?"repeat(2,1fr)":"repeat(4,1fr)",gap:10,marginBottom:18}}>
        <Card style={{padding:"10px 12px",borderTop:"3px solid "+colors.gn}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:16}}>✅</span><span style={{fontSize:17,fontWeight:800,color:colors.gn}}>${totalApr.toLocaleString()}</span></div><div style={{fontSize:10,color:colors.g4,marginTop:3}}>Total Aprobado</div></Card>
        <Card style={{padding:"10px 12px",borderTop:"3px solid "+colors.yl}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:16}}>⏳</span><span style={{fontSize:17,fontWeight:800,color:colors.yl}}>{pendientes}</span></div><div style={{fontSize:10,color:colors.g4,marginTop:3}}>Pendientes</div></Card>
        <Card style={{padding:"10px 12px",borderTop:"3px solid "+colors.bl}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:16}}>📊</span><span style={{fontSize:17,fontWeight:800,color:colors.bl}}>{presu.length}</span></div><div style={{fontSize:10,color:colors.g4,marginTop:3}}>Total Cotizaciones</div></Card>
        <Card style={{padding:"10px 12px",borderTop:"3px solid "+colors.pr}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:16}}>⏱️</span><span style={{fontSize:17,fontWeight:800,color:colors.pr}}>{avgDays}d</span></div><div style={{fontSize:10,color:colors.g4,marginTop:3}}>Tiempo resp. prom.</div></Card>
      </div>
      {/* Auto PDF Report */}
      ...
      {areaGasto.length>0&&<Card style={{padding:14,marginBottom:14}}>
        <div style={{fontSize:13,fontWeight:700,color:colors.nv,marginBottom:10}}>Gasto aprobado por Área</div>
        {areaGasto.map((a:any)=><div key={a.id} style={{marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3}}><span style={{fontWeight:600,color:colors.nv}}>{a.icon} {a.name}</span><span style={{fontWeight:700,color:colors.gn}}>${a.gasto.toLocaleString()}</span></div>
          <div style={{height:8,background:colors.g2,borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:Math.round(a.gasto/maxGasto*100)+"%",background:a.color,borderRadius:4}}/></div>
        </div>)}
      </Card>}
      {/* Tendencia mensual */}
      ...
      return monthData.some((m:any)=>m.count>0)?<Card style={{padding:14,marginBottom:14}}>
        <div style={{fontSize:13,fontWeight:700,color:colors.nv,marginBottom:10}}>📈 Tendencia últimos 6 meses</div>
        <div style={{display:"flex",gap:mob?4:8,alignItems:"flex-end",height:120}}>
          {monthData.map((m:any,i:number)=><div key={i} style={{flex:1,display:"flex",flexDirection:"column" as const,alignItems:"center",gap:2}}>
            <span style={{fontSize:9,fontWeight:700,color:colors.gn}}>{m.apr>0?"$"+Math.round(m.apr/1000)+"k":""}</span>
            <div style={{width:"100%",display:"flex",flexDirection:"column" as const,gap:1,justifyContent:"flex-end",height:80}}>
              <div style={{height:Math.max(2,Math.round(m.apr/maxM*80)),background:colors.gn,borderRadius:3}} title={"Aprobado: $"+m.apr.toLocaleString()}/>
              <div style={{height:Math.max(2,Math.round((m.sol-m.apr)/maxM*80)),background:colors.yl+"60",borderRadius:3}} title={"Pendiente: $"+(m.sol-m.apr).toLocaleString()}/>
            </div>
            <span style={{fontSize:9,color:colors.g4,fontWeight:600}}>{m.label}</span>
            <span style={{fontSize:8,color:colors.g5}}>{m.count}</span>
          </div>)}
        </div>
        <div style={{display:"flex",gap:12,marginTop:8,fontSize:10}}><span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:colors.gn}}/> Aprobado</span><span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:colors.yl+"60"}}/> Pendiente</span></div>
      </Card>:null;})()}
      {/* Top proveedores */}
      ...
      return top.length>0?<Card style={{padding:14,marginBottom:14}}>
        <div style={{fontSize:13,fontWeight:700,color:colors.nv,marginBottom:10}}>🏢 Top 5 Proveedores (aprobado)</div>
        {top.map((pv:any,i:number)=><div key={i} style={{marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3}}><span style={{fontWeight:600,color:colors.nv}}>{i+1}. {pv.nombre} <span style={{color:colors.g4,fontWeight:400}}>({pv.count} cotiz.)</span></span><span style={{fontWeight:700,color:colors.pr}}>${pv.total.toLocaleString()}</span></div>
          <div style={{height:6,background:colors.g2,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:Math.round(pv.total/maxProv*100)+"%",background:colors.pr,borderRadius:3}}/></div>
        </div>)}
      </Card>:null;})()}
      {/* Carga de trabajo */}
      ...
      return sorted.length>0?<Card style={{padding:14}}>
        <div style={{fontSize:13,fontWeight:700,color:colors.nv,marginBottom:10}}>👥 Carga de trabajo (tareas activas)</div>
        {sorted.map((w:any,i:number)=><div key={i} style={{marginBottom:6}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:2}}><span style={{fontWeight:600,color:colors.nv}}>{w.name}</span><span style={{display:"flex",gap:6}}><span style={{fontWeight:700,color:colors.yl}}>{w.active} activas</span>{w.overdue>0&&<span style={{fontWeight:700,color:"#DC2626"}}>⏰ {w.overdue}</span>}<span style={{color:colors.g4}}>{w.total} total</span></span></div>
          <div style={{height:6,background:colors.g2,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:Math.round(w.active/maxW*100)+"%",background:w.overdue>0?"#DC2626":w.active>5?colors.yl:colors.gn,borderRadius:3}}/></div>
        </div>)}
      </Card>:null;})()}
    </div>);
  }
