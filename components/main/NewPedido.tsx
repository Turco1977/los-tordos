...
export function NP({user,onSub,onX,preAssign,preArea,preDept,mob,canjeUsado}:any){
  ...
  return(<Card style={{maxWidth:mob?undefined:560}}>
    ...
    {isHigh&&<><div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:8,marginBottom:10}}><div><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Área</label><select value={f.aId} onChange={(e:any)=>{sF((p:any)=>({...p,aId:e.target.value,dId:""}));}} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">Todas</option>{areas.map((a:any)=><option key={a.id} value={a.id}>{a.icon} {a.name}</option>)}</select></div><div><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Departamento</label><select value={f.dId} onChange={(e:any)=>{up("dId",e.target.value);const dept=deptos.find((d:any)=>d.id===Number(e.target.value));if(dept&&(dept.aId===100||dept.aId===101)){const dN=dept.name.toLowerCase();const member=(om||[]).find((m:any)=>m.cargo&&m.cargo.toLowerCase()===dN);if(member){const mu=users.find((u:any)=>u.mail&&member.mail&&u.mail===member.mail)||users.find((u:any)=>u.n===member.n&&u.a===member.a);if(mu){sAssignedUser(mu);sNameSearch(fn(mu));return;}}sAssignedUser(null);sNameSearch("");}}} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">General</option>{(selArea?deptos.filter((d:any)=>d.aId===selArea.id):deptos).map((d:any)=><option key={d.id} value={d.id}>{d.name}</option>)}</select></div></div><div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>División</label><select value={f.div} onChange={(e:any)=>up("div",e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">General</option>{DIV.map(d=><option key={d} value={d}>{d}</option>)}</select></div>{!preAssign&&<div style={{marginBottom:10,position:"relative"}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Asignar a</label><input value={nameSearch} onChange={(e:any)=>{sNameSearch(e.target.value);if(!e.target.value){sAssignedUser(null);}}} placeholder="Buscar nombre..." style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+(assignedUser?T.gn:T.g3),fontSize:12,boxSizing:"border-box" as const,marginTop:3,background:assignedUser?"#D1FAE520":"#fff"}}/>{nameSearch&&!assignedUser&&users.filter((u:any)=>fn(u).toLowerCase().includes(nameSearch.toLowerCase())).length>0&&<div style={{position:"absolute",zIndex:10,width:"100%",border:"1px solid "+T.g3,borderRadius:8,marginTop:2,maxHeight:120,overflowY:"auto" as const,background:"#fff",boxShadow:"0 4px 12px #0002"}}>{users.filter((u:any)=>fn(u).toLowerCase().includes(nameSearch.toLowerCase())).slice(0,8).map((u:any)=><div key={u.id} onClick={()=>{sAssignedUser(u);sNameSearch(fn(u));}} style={{padding:"6px 10px",fontSize:11,cursor:"pointer",borderBottom:"1px solid "+T.g1}}>{fn(u)}{u.div?" · "+u.div:""}</div>)}</div>}{assignedUser&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}><span style={{fontSize:10,color:T.gn,fontWeight:600}}>Asignado a {fn(assignedUser)}</span><button onClick={()=>{sAssignedUser(null);sNameSearch("");}} style={{background:"none",border:"none",cursor:"pointer",fontSize:10,color:T.g4}}>x</button></div>}</div>}</>}
    ...
    <div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Título *</label><input value={f.tit} onChange={(e:any)=>up("tit",e.target.value)} placeholder="Título corto (ej: Compra Pelotas)" style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:3}}/></div>
    <div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Descripción *</label><MentionInput users={users} value={f.desc} onChange={(val:string)=>up("desc",val)} rows={3} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,resize:"vertical" as const,boxSizing:"border-box" as const,marginTop:3}}/></div>
