...
export function SB({aA,aD,onAC,onDC,col,onCol,isPersonal,mob,sbOpen,onClose,vw,onNav,user}:any){
  ...
  const canVoteCd=(()=>{if(!user)return false;return user.role==="superadmin"||user.role==="admin"||userAreaIds.includes(100);})();
  const projPending=canVoteCd?(projects||[]).filter((p:any)=>p.status==="enviado"&&!(p.votos||[]).some((v:any)=>v.userId===user.id)).length:0;
  const presuPending=(()=>{if(!user)return 0;const isCompras=user.dId===7||user.role==="embudo";const isTes=user.dId===61||user.dId===53;const isSe=userAreaIds.includes(101)||user.role==="superadmin"||user.role==="admin";return(presu||[]).filter((p:any)=>{const votos=p.votos||[];const yaVoto=votos.some((v:any)=>v.userId===user.id);if(yaVoto)return false;if(p.status==="solicitado"&&(isCompras||isTes))return true;if(p.status==="pendiente_se"&&isSe)return true;return false;}).length;})();
  const sbContent=(<div style={{flex:1,overflowY:"auto" as const,padding:"8px 6px"}}>
    ...
    {onNav&&<div style={{marginTop:10,padding:"4px 2px"}}>
      ...
      {[
        ...
        {k:"reun",l:"Reuniones",icon:"🤝",show:!isPersonal&&user&&(user.role==="admin"||user.role==="superadmin"||user.role==="coordinador"||isCdOrSe||isMesaConvivencia)},
