    ...
    <div onClick={(e:any)=>e.stopPropagation()} style={{background:cardBg,borderRadius:mob?0:14,maxWidth:mob?undefined:640,width:"100%",height:mob?"100vh":"85vh",display:"flex",flexDirection:"column" as const,overflow:"hidden"}}>
      ...
      <div style={{flex:1,padding:"12px 20px",overflow:"auto",display:"flex",flexDirection:"column" as const}}>
        ...
        {tab==="info"&&<div>
          ...
          {p.desc&&<div style={{marginBottom:12,padding:12,background:isDark?"rgba(255,255,255,.03)":colors.g1,borderRadius:10,border:"1px solid "+colors.g2}}>
            <div style={{fontSize:9,color:colors.g4,fontWeight:700,marginBottom:4}}>DESCRIPCIÓN</div>
            <div style={{fontSize:13,color:colors.nv,lineHeight:1.6,whiteSpace:"pre-wrap" as const}}>{renderMentions(p.desc)}</div>
          </div>}
        ...
        {tab==="acc"&&<div style={{display:"flex",flexDirection:"column" as const,gap:8}}>
          ...
          {(isM||isSA)&&(p.st===ST.C||p.st===ST.P)&&<div style={{display:"flex",flexDirection:"column" as const,gap:6}}>
            ...
            <MentionInput users={users} value={rp} onChange={sRp} rows={3} placeholder="Escribí la resolución acá..." style={{padding:8,borderRadius:8,border:"1px solid "+(rp.trim()?colors.gn:colors.g3),fontSize:12,resize:"vertical" as const,outline:"none"}}/>
            {!rp.trim()&&<div style={{fontSize:10,color:colors.rd,fontWeight:600}}>⚠️ Requerido: escribí una resolución para poder finalizar la tarea</div>}
            {p.rG&&!(p.eOk&&p.tOk)&&rp.trim()&&<div style={{fontSize:10,color:"#92400E",fontWeight:600,padding:"6px 8px",background:"#FEF3C7",borderRadius:6}}>💰 Esta tarea requiere aprobación de Compras y Tesorería antes de finalizar</div>}
        ...
        {tab==="check"&&<div style={{display:"flex",flexDirection:"column" as const,gap:8}}>
          ...
          <div style={{display:"flex",gap:4}}><MentionInput as="input" users={users} value={newChk} onChange={v=>sNewChk(v)} onKeyDown={e=>{if(e.key==="Enter"&&newChk.trim()){const nu=[...chkItems,{text:newChk.trim(),done:false}];sChkItems(nu);sNewChk("");if(onCheck)onCheck(p.id,nu);}}} placeholder="Nuevo item..." style={{width:"100%",padding:"6px 10px",borderRadius:6,border:"1px solid "+colors.g3,fontSize:12,boxSizing:"border-box" as const}} containerStyle={{flex:1}}/><Btn v="p" s="s" disabled={!newChk.trim()} onClick={()=>{const nu=[...chkItems,{text:newChk.trim(),done:false}];sChkItems(nu);sNewChk("");if(onCheck)onCheck(p.id,nu);}}>+</Btn></div>
