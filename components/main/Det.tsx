    ...
    <div onClick={(e:any)=>e.stopPropagation()} style={{background:cardBg,borderRadius:mob?0:14,maxWidth:mob?undefined:640,width:"100%",height:mob?"100vh":"85vh",display:"flex",flexDirection:"column" as const,overflow:"hidden"}}>
      ...
      <div style={{flex:1,padding:"12px 20px",overflow:"auto",display:"flex",flexDirection:"column" as const}}>
        ...
        {tab==="check"&&<div style={{display:"flex",flexDirection:"column" as const,gap:8}}>
          ...
          <div style={{display:"flex",gap:4}}><MentionInput as="input" users={users} value={newChk} onChange={v=>sNewChk(v)} onKeyDown={e=>{if(e.key==="Enter"&&newChk.trim()){const nu=[...chkItems,{text:newChk.trim(),done:false}];sChkItems(nu);sNewChk("");if(onCheck)onCheck(p.id,nu);}}} placeholder="Nuevo item..." style={{flex:1,padding:"6px 10px",borderRadius:6,border:"1px solid "+colors.g3,fontSize:12}} containerStyle={{flex:1}}/><Btn v="p" s="s" disabled={!newChk.trim()} onClick={()=>{const nu=[...chkItems,{text:newChk.trim(),done:false}];sChkItems(nu);sNewChk("");if(onCheck)onCheck(p.id,nu);}}>+</Btn></div>
