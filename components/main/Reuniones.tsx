...
import { T, AGT, MINSECS, DEPTOS, ROLES, fn } from "@/lib/constants";
...
export function Reuniones({agendas,minutas,om,users,areas,onAddAg,onUpdAg,onDelAg,onAddMin,onUpdMin,onDelMin,onCreateTasks,user,mob}:any){
  ...
  if(mode==="home") return(<div style={{maxWidth:720}}>
    <h2 style={{margin:"0 0 4px",fontSize:19,color:T.nv,fontWeight:800}}>{"\u{1F4C5}"} Reuniones</h2>
    <p style={{color:T.g4,fontSize:12,margin:"0 0 14px"}}>{"\u00D3"}rdenes del d{"\u00ED"}a y minutas institucionales</p>
    ...
    <Card style={{marginBottom:14,borderLeft:"4px solid "+tmpl.color,padding:"12px 16px"}}>
      ...
      <div style={{fontSize:11,color:T.g4}}>Periodicidad: {tmpl.per} {"\u00B7"} Duraci{"\u00F3"}n: {tmpl.dur}</div>
    </Card>
    <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:10,marginBottom:18}}>
      <Card style={{padding:mob?"14px 12px":"18px 16px",cursor:"pointer",textAlign:"center" as const,border:"2px solid "+T.g2}} onClick={startNewAg}>
        <span style={{fontSize:28}}>{"\u{1F4CB}"}</span><div style={{fontSize:13,fontWeight:700,color:T.nv,marginTop:6}}>Nueva Orden del D{"\u00ED"}a</div><div style={{fontSize:10,color:T.g4}}>Crear agenda para pr{"\u00F3"}xima reuni{"\u00F3"}n</div>
      </Card>
      <Card style={{padding:"18px 16px",cursor:"pointer",textAlign:"center" as const,border:"2px solid "+T.g2}} onClick={()=>startNewMin()}>
        <span style={{fontSize:28}}>{"\u{1F4DD}"}</span><div style={{fontSize:13,fontWeight:700,color:T.nv,marginTop:6}}>Nueva Minuta</div><div style={{fontSize:10,color:T.g4}}>Registrar acta de reuni{"\u00F3"}n</div>
      </Card>
    </div>
    <div style={{fontSize:13,fontWeight:700,color:T.nv,marginBottom:8}}>{"\u{1F4DA}"} Historial</div>
    {fAg.length===0&&fMi.length===0&&<Card style={{textAlign:"center" as const,padding:24,color:T.g4}}><span style={{fontSize:24}}>{"\u{1F4ED}"}</span><div style={{marginTop:6,fontSize:12}}>Sin registros a{"\u00FA"}n</div></Card>}
    {fAg.length>0&&<div style={{marginBottom:12}}><div style={{fontSize:11,fontWeight:700,color:T.g4,marginBottom:4}}>{"\u{1F4CB}"} {"\u00D3"}RDENES DEL D{"\u00CD"}A</div>
      {fAg.map((a:any)=><Card key={a.id} style={{padding:"10px 14px",marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{flex:1,cursor:"pointer"}} onClick={()=>{sSelId(a.id);sMode("viewOD");}}><div style={{fontSize:12,fontWeight:600,color:T.nv}}>{"\u{1F4CB}"} Orden del D{"\u00ED"}a {"\u2013"} {fmtD(a.date)}{a.areaName?" · "+a.areaName:""}</div><div style={{fontSize:10,color:T.g4}}>{a.status==="enviada"?"\u2705 Enviada":"\u{1F4DD} Borrador"} {"\u00B7"} Creada: {fmtD(a.createdAt)}</div></div>
        <div style={{display:"flex",gap:4,alignItems:"center"}}><button onClick={(e)=>{e.stopPropagation();if(confirm("\u00BFEliminar esta Orden del D\u00EDa?"))onDelAg(a.id);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:T.rd,padding:4}} title="Eliminar">{"\u{1F5D1}\uFE0F"}</button><span style={{color:T.g4}}>{"\u203A"}</span></div>
      </Card>)}</div>}
    {fMi.length>0&&<div><div style={{fontSize:11,fontWeight:700,color:T.g4,marginBottom:4}}>{"\u{1F4DD}"} MINUTAS</div>
      {fMi.map((m:any)=><Card key={m.id} style={{padding:"10px 14px",marginBottom:4,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{flex:1,cursor:"pointer"}} onClick={()=>{sSelId(m.id);sMode("viewMin");}}><div style={{fontSize:12,fontWeight:600,color:T.nv}}>{"\u{1F4DD}"} Minuta {"\u2013"} {fmtD(m.date)}{m.areaName?" · "+m.areaName:""}</div><div style={{fontSize:10,color:T.g4}}>{m.status==="final"?"\u2705 Finalizada":"\u{1F4DD} Borrador"}{m.tareas?.length?" · 📋 "+m.tareas.length+" tareas":""}</div></div>
        <div style={{display:"flex",gap:4,alignItems:"center"}}><button onClick={(e)=>{e.stopPropagation();if(confirm("\u00BFEliminar esta Minuta?"))onDelMin(m.id);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:T.rd,padding:4}} title="Eliminar">{"\u{1F5D1}\uFE0F"}</button><span style={{color:T.g4}}>{"\u203A"}</span></div>
      </Card>)}</div>}
  </div>);
  ...
  const renderSecEditor=()=>{
    ...
    return(<>
      <div style={{fontSize:12,fontWeight:700,color:T.nv,marginBottom:8}}>Estructura del Orden del D{"\u00ED"}a</div>
      ...
      <button onClick={()=>sAgSecs(p=>[...p,{t:"",sub:[],notes:"",atts:[]}])} style={{width:"100%",padding:8,borderRadius:8,border:"2px dashed "+T.g3,background:"transparent",cursor:"pointer",fontSize:11,fontWeight:600,color:T.bl,marginBottom:12}}>+ Agregar secci{"\u00F3"}n</button>
      {/* Asistencia */}
  ...
  if(mode==="newOD"){
    ...
    return(<div style={{maxWidth:640}}>
      ...
      <Card>
        <h2 style={{margin:"0 0 14px",fontSize:17,color:T.nv,fontWeight:800}}>{"\u{1F4CB}"} Nueva Orden del D{"\u00ED"}a {"\u2013"} {tmpl.title}</h2>
        <div style={{padding:8,background:T.g1,borderRadius:8,fontSize:11,color:T.g5,marginBottom:12}}>Periodicidad: {tmpl.per} {"\u00B7"} Duraci{"\u00F3"}n: {tmpl.dur}</div>
        <div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Fecha de reuni{"\u00F3"}n</label><input type="date" value={agDate} onChange={e=>sAgDate(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:3}}/></div>
        {tab==="area"&&<div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>{"\u00C1"}rea</label><select value={areaName} onChange={e=>{sAreaName(e.target.value);sDeptName("");}} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">Seleccionar...</option>{areas.filter((a:any)=>a.id!==100&&a.id!==101).map((a:any)=><option key={a.id} value={a.name}>{a.icon} {a.name}</option>)}</select></div>}
        {tab==="area"&&areaName&&areaDepts.length>0&&<div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Departamento</label><select value={deptName} onChange={e=>sDeptName(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">Todos los departamentos</option>{areaDepts.map((d:any)=><option key={d.id} value={d.name}>{d.name}</option>)}</select></div>}
        {renderSecEditor()}
  ...
  if(mode==="editOD"){
    ...
    return(<div style={{maxWidth:640}}>
      ...
      <Card>
        <h2 style={{margin:"0 0 14px",fontSize:17,color:T.nv,fontWeight:800}}>{"\u270F\uFE0F"} Editar Orden del D{"\u00ED"}a{ag.areaName?" – "+ag.areaName:""}</h2>
        <div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Fecha de reuni{"\u00F3"}n</label><input type="date" value={agDate} onChange={e=>sAgDate(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,boxSizing:"border-box" as const,marginTop:3}}/></div>
        {tab==="area"&&<div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>{"\u00C1"}rea</label><select value={areaName} onChange={e=>{sAreaName(e.target.value);sDeptName("");}} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">Seleccionar...</option>{areas.filter((a:any)=>a.id!==100&&a.id!==101).map((a:any)=><option key={a.id} value={a.name}>{a.icon} {a.name}</option>)}</select></div>}
        {tab==="area"&&areaName&&areaDepts.length>0&&<div style={{marginBottom:10}}><label style={{fontSize:12,fontWeight:600,color:T.g5}}>Departamento</label><select value={deptName} onChange={e=>sDeptName(e.target.value)} style={{width:"100%",padding:8,borderRadius:8,border:"1px solid "+T.g3,fontSize:12,marginTop:3}}><option value="">Todos los departamentos</option>{areaDepts.map((d:any)=><option key={d.id} value={d.name}>{d.name}</option>)}</select></div>}
        {renderSecEditor()}
    ...
    return(<div style={{maxWidth:640}}>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:14}}>
          <div><h2 style={{margin:0,fontSize:17,color:T.nv,fontWeight:800}}>{"\u{1F4CB}"} Orden del D{"\u00ED"}a{ag.areaName?" – "+ag.areaName:""}</h2><div style={{fontSize:11,color:T.g4,marginTop:2}}>Fecha: {fmtD(ag.date)} {"\u00B7"} {ag.status==="enviada"?"\u2705 Enviada":"\u{1F4DD} Borrador"}</div></div>
        ...
        <div style={{borderTop:"2px solid "+T.nv,paddingTop:12}}>
          <div style={{textAlign:"center" as const,marginBottom:12}}>
            ...
            <div style={{fontSize:11,color:T.g5}}>Orden del D{"\u00ED"}a {"\u2013"} {fmtD(ag.date)}</div>
          </div>
          {(ag.sections||[]).map((s:any,i:number)=><div key={i} style={{marginBottom:8,padding:"8px 10px",background:i%2===0?"#FAFAFA":"#fff",borderRadius:6}}>
            ...
            {s.notes&&<div style={{fontSize:11,color:T.bl,marginTop:3,fontStyle:"italic",paddingLeft:12}}>{"\u{1F4AC}"} {s.notes}</div>}
  ...
  if(mode==="newMin"){
    ...
    return(<div style={{maxWidth:640}}>
      <Btn v="g" s="s" onClick={()=>sMode("home")} style={{marginBottom:12}}>{"\u2190"} Volver</Btn>
      <Card>
        <h2 style={{margin:"0 0 14px",fontSize:17,color:T.nv,fontWeight:800}}>{"\u{1F4DD}"} Nueva Minuta {"\u2013"} {tmpl.title}</h2>
        ...
        {tab==="area"&&<div style={{marginBottom:10}}><label style={{fontSize:11,fontWeight:600,color:T.g5}}>{"\u00C1"}rea</label><select value={areaName} onChange={e=>{sAreaName(e.target.value);sDeptName("");}} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+T.g3,fontSize:12,marginTop:2}}><option value="">Seleccionar...</option>{areas.filter((a:any)=>a.id!==100&&a.id!==101).map((a:any)=><option key={a.id} value={a.name}>{a.icon} {a.name}</option>)}</select></div>}
        {tab==="area"&&areaName&&areaDepts.length>0&&<div style={{marginBottom:10}}><label style={{fontSize:11,fontWeight:600,color:T.g5}}>Departamento</label><select value={deptName} onChange={e=>sDeptName(e.target.value)} style={{width:"100%",padding:7,borderRadius:7,border:"1px solid "+T.g3,fontSize:12,marginTop:2}}><option value="">Todos los departamentos</option>{areaDepts.map((d:any)=><option key={d.id} value={d.name}>{d.name}</option>)}</select></div>}
        {members.length>0&&<div style={{marginBottom:10}}>
        ...
        <div style={{marginTop:12,padding:12,background:"#FEF3C7",borderRadius:10,border:"1px solid #FDE68A"}}>
          ...
          {miTareas.length===0&&<div style={{fontSize:11,color:T.g4,textAlign:"center" as const,padding:8}}>Sin tareas. Se crear{"\u00E1"}n autom{"\u00E1"}ticamente al finalizar la minuta.</div>}
  ...
  if(mode==="editMin"){
    ...
    return(<div style={{maxWidth:640}}>
      ...
      <Card>
        ...
        <div style={{marginTop:12,padding:12,background:"#FEF3C7",borderRadius:10,border:"1px solid #FDE68A"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:12,fontWeight:700,color:"#92400E"}}>{"\u{1F4CB}"} Tareas asignadas</div><Btn v="w" s="s" onClick={()=>sMiTareas(p=>[...p,{desc:"",respId:"",fecha:""}])}>+ Agregar tarea</Btn></div>
          {miTareas.length===0&&<div style={{fontSize:11,color:T.g4,textAlign:"center" as const,padding:8}}>Sin tareas. Agreg{"\u00E1"} tareas y finaliz{"\u00E1"} para crearlas en el sistema.</div>}
          {miTareas.map((t:any,i:number)=><div key={i} style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr auto auto auto",gap:6,marginBottom:6,alignItems:"end"}}>
            <div><label style={{fontSize:9,color:T.g5}}>Tarea</label><input value={t.desc} onChange={e=>{const n=[...miTareas];n[i]={...n[i],desc:e.target.value};sMiTareas(n);}} placeholder="Descripci\u00F3n..." style={{width:"100%",padding:6,borderRadius:6,border:"1px solid "+T.g3,fontSize:11,boxSizing:"border-box" as const}}/></div>
            <div><label style={{fontSize:9,color:T.g5}}>Responsable</label><select value={t.respId} onChange={e=>{const n=[...miTareas];n[i]={...n[i],respId:e.target.value};sMiTareas(n);}} style={{padding:6,borderRadius:6,border:"1px solid "+T.g3,fontSize:11}}><option value="">Seleccionar...</option>{stf.map((u:any)=><option key={u.id} value={u.id}>{fn(u)}</option>)}</select></div>
            <div><label style={{fontSize:9,color:T.g5}}>Fecha</label><input type="date" value={t.fecha} onChange={e=>{const n=[...miTareas];n[i]={...n[i],fecha:e.target.value};sMiTareas(n);}} style={{padding:6,borderRadius:6,border:"1px solid "+T.g3,fontSize:11}}/></div>
    ...
    return(<div style={{maxWidth:640}}>
      ...
      <Card>
        ...
        <div style={{textAlign:"center" as const,borderBottom:"2px solid "+T.nv,paddingBottom:12,marginBottom:12}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:12,marginTop:4,fontSize:11,color:T.g5}}><span>{"\u{1F4C5}"} {fmtD(mi.date)}</span>{mi.horaInicio&&<span>{"\u{1F550}"} {mi.horaInicio} {"\u2013"} {mi.horaCierre}</span>}{mi.lugar&&<span>{"\u{1F4CD}"} {mi.lugar}</span>}</div>
