...
function StaffFichaModal({staff,canEdit,onClose,onSave,mob}:{staff:any;canEdit:boolean;onClose:()=>void;onSave:(d:any)=>void;mob:boolean}){
  const [editing,sEditing]=useState(false);
  const [form,sForm]=useState({first_name:staff.first_name||"",last_name:staff.last_name||"",dep_role:staff.dep_role||"entrenador",divisions:staff.divisions||[] as string[],active:staff.active!==false});
...
export function Org({areas,deptos,users,om,onEditSave,onDelOm,onDelUser,onEditUser,isSA,onAssignTask,onDm,mob,pedidos,onSel,KPIs,Circles,DeptCircles,TList,onReorderOm,onReorderUser,showT}:any){
  ...
  const fetchDepStaff=useCallback(async()=>{
    ...
    const{data:profiles}=await sb.from("profiles").select("id,first_name,last_name,role,email").in("id",uids);
    const pMap=new Map((profiles||[]).map((p:any)=>[p.id,p]));
    sDepStaff(staffRows.map((s:any)=>{const pr=pMap.get(s.user_id);return{...s,first_name:pr?.first_name||"",last_name:pr?.last_name||"",email:pr?.email||""};}));
  },[]);
  ...
  const handleStaffSave=async(d:any)=>{
    try{
      const sb=createClient();
      await sb.from("profiles").update({first_name:d.first_name,last_name:d.last_name}).eq("id",d.user_id);
  ...
  return(<div style={{maxWidth:mob?undefined:720}}>
    ...
    {selStaff&&<StaffFichaModal staff={selStaff} canEdit={!!isSA} onClose={()=>sSelStaff(null)} onSave={handleStaffSave} mob={mob}/>}
  </div>);
}
